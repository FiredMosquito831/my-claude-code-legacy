...
**A model's first-token allowance is the smaller of the deadline and its share of the budget — and since 6.10.0 that share has a floor.** Each attempt gets an equal share of whatever is left of the total budget, counting itself and every model still behind it on the chain; that share is then raised to `FALLBACK_ATTEMPT_SHARE_FLOOR` if it came out lower (never above what is actually left), and what is applied is the smaller of the result and the first-token deadline.

Before the floor the share alone decided: with 600s total and a 120s deadline, a ten-model route gave the first model `min(120, 600 ÷ 10)` = **60s**, and the 120 in the box never applied to that route at all. That is what produced log lines like `produced no first token after 74.9494s` on an eight-model chain — a number that appeared nowhere in the configuration. With the shipped defaults (600s total, 180s first token, 180s floor) the same route gives every silent model the full **180s**: `min(180, max(600 ÷ 10, 180))`.

The floor buys that honesty with the models behind it: ten models at a 180s floor is 1,800s of demand against a 600s budget, so only the first three silent models can use the whole floor and the ones after them get whatever is left, then nothing. That is the operator's trade to make. Set `FALLBACK_ATTEMPT_SHARE_FLOOR=0` for the old pure equal-share, or raise the total budget to `models × floor` to fit them all. The Deadlines calculator on this page computes both numbers for your own chains and warns when the floor cannot fit.
