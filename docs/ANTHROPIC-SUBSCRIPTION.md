...
Measured on real traffic: the terminal CLI reports `cc_entrypoint=cli`, and the
Python Agent SDK reports `cc_entrypoint=sdk-py`. Because the marker travels in
the body, a proxy can neither forge it for traffic it did not receive nor strip
it from traffic it did.

**By default, `anthropic_oauth` refuses any request that does not report
`cc_entrypoint=cli`.** The Agent SDK, other harnesses, and bare API calls never
touch the subscription credential; they are refused with a message pointing at
the `anthropic` provider instead.

This matters because the Agent SDK is the one case Anthropic's policy names
explicitly. Without the gate, pointing any SDK script at your proxy would
...
- It does **not** hide anything from Anthropic. MCC sends the Claude Code
  header set, including `x-app: cli` and a `claude-cli` User-Agent, because the
...
| --- | --- | --- |
| `ANTHROPIC_OAUTH_REQUIRE_CLAUDE_CODE` | `true` | Refuse requests not reporting `cc_entrypoint=cli` |
| `ANTHROPIC_OAUTH_ACCESS_TOKEN` | *(empty)* | Raw token override. **Cannot be refreshed** — it will expire and stay expired. Prefer the login. |
