...
def test_every_pre_adaptive_preference_is_byte_identical(
    ...
    Eleven ``anthropic_messages`` cells were deliberately re-baselined in
    5.64.0: where the thinking budget met or exceeded the client's
    ``max_tokens``, the encoder used to raise ``max_tokens`` to ``budget + 1``,
    so a client asking for 4,096 output tokens at the ``max`` tier was silently
