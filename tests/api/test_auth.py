...
def test_proxy_auth_accepts_the_two_headers_an_anthropic_client_actually_sends():
    ...
    with patch("my_claude_code.api.routes.get_token_count", return_value=1):
...
def test_authorization_still_wins_when_both_headers_are_present():
    ...
    with patch("my_claude_code.api.routes.get_token_count", return_value=2):
        r = client.post(
            "/v1/messages/count_tokens",
            json=payload,
            headers={
...
def test_anthropic_auth_token_normalizes_configured_whitespace():
    ...
    with patch("my_claude_code.api.routes.get_token_count", return_value=3):
        r = client.post(
            "/v1/messages/count_tokens",
            json=payload,
