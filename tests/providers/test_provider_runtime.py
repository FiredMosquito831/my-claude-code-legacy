...
from my_claude_code.providers.runtime import (
    ProviderRuntime,
...
def test_importing_runtime_does_not_eager_load_other_adapters() -> None:
    ...
    code = (
        "import sys\n"
        "import my_claude_code.providers.runtime\n"
        "assert 'my_claude_code.providers.open_router' not in sys.modules\n"
    )
    proc = subprocess.run(
...
def test_create_provider_instantiates_each_builtin():
    settings = _make_settings(
        ...
        hypercharm_api_key="test_hypercharm_key",
    ...
    with (
        ...
        patch(
            "my_claude_code.providers.runtime.factory.ProviderRateLimiter",
