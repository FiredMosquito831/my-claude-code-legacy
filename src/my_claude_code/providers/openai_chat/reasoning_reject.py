...
The counterpart to ``output_cap.py``. That module learns a number a provider
named; this one learns a *field* a provider refused. Both exist because the
alternative -- a hand-written per-provider verdict -- cannot be right for a
gateway that fronts many models, which is precisely what the 5.70.0
``NO_REASONING`` audit discovered the hard way.
