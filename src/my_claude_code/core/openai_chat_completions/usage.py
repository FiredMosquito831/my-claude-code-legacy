"""Anthropic usage translated into the Chat Completions usage object.

The two protocols count the same tokens and name them differently, and one of
the differences is not cosmetic: Anthropic's ``input_tokens`` *excludes* what
the prompt cache served, while OpenAI's ``prompt_tokens`` includes it and then
says how much of it was cached in ``prompt_tokens_details.cached_tokens``. A
straight rename would therefore under-report the prompt of every cached
request -- which is most of them, for a coding agent.
"""
...
class ChatUsageLedger:
    ...
    def record_message_start(self, data: Mapping[str, Any]) -> None:
        ...
        the final ``message_delta``; without this the prompt figure would be
        that zero.
        """
    ...
    def payload(self) -> dict[str, Any]:
        """Return the OpenAI ``usage`` object for what was counted."""
        prompt_tokens = (
            (self._input_tokens or 0)
            + self._cache_read_tokens
            + self._cache_write_tokens
        )
        completion_tokens = self._output_tokens or 0
        usage: dict[str, Any] = {
        ...
        if self._cache_read_tokens:
            usage["prompt_tokens_details"] = {"cached_tokens": self._cache_read_tokens}
        ...
        reasoning_tokens = min(self._reasoning_tokens_estimate, completion_tokens)
        if reasoning_tokens:
