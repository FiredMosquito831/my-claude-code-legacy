"""Translate Anthropic SSE streams into Chat Completions SSE streams."""
...
from .assembler import ChatCompletionsStreamAssembler
...
async def iter_chat_sse_from_anthropic(
    ...
    *,
    ...
    """Yield Chat Completions SSE frames translated from an Anthropic stream.
    ...
    object inside the stream.
    """
    assembler = ChatCompletionsStreamAssembler(request)
    assembler.bind_completion_id(completion_id)
    ...
    try:
        ...
        await close_stream_input(
            events,
            owner="openai_chat_completions.stream",
            source="core",
