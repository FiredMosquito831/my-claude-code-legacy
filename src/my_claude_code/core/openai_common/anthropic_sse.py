"""Anthropic SSE parsing shared by the OpenAI-compatible stream adapters.

Both OpenAI ingress surfaces receive the same thing from the provider path --
an Anthropic SSE byte stream -- and have to turn it into discrete events
before they can translate anything. That reader is protocol-neutral, so it
lives here rather than inside either adapter.
"""
...
async def iter_sse_events(
    ...
    try:
        ...
        await close_stream_input(
            iterator,
            owner="openai_common.anthropic_sse",
            source="core",
