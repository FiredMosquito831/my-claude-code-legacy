...
**A silent model counts as a failure too.** A provider that accepts the request and then produces nothing would otherwise hold it until the transport read timeout — minutes — and the chain would get its turn long after the client gave up. These bound it, on **Admin UI → Limits & Resilience** (deadlines on the **Deadlines** card, the last two on **Chain benching**):

| Setting | Default | What it does |
| --- | --- | --- |
| `FALLBACK_FIRST_TOKEN_TIMEOUT` | `180` | The first-token deadline: seconds a model may stay silent before the next model takes over. Nothing has streamed yet, so the handover is invisible. `0` waits indefinitely. Each attempt still gets an equal share of what is left of `FALLBACK_TOTAL_TIMEOUT`, counting itself and every model still behind it, and what applies is whichever is smaller — but since 6.10.0 that share cannot fall below `FALLBACK_ATTEMPT_SHARE_FLOOR`, so with the shipped defaults the number in this box **is** the number a silent model gets. **Admin UI → Limits & Resilience** computes it per route for your own chains. |
| `FALLBACK_ATTEMPT_SHARE_FLOOR` | `180` | Smallest slice of `FALLBACK_TOTAL_TIMEOUT` one attempt may be cut down to. Without it the equal share alone decided the allowance, and on a long chain it silently undercut the deadline above: 600 ÷ 8 models = 75s, so a box reading `120` logged `produced no first token after 74.9494s`. Set equal to the first-token deadline by default, so the pair honours itself out of the box. The trade: N silent models can spend up to N × this floor before the budget clamps them, and the models after that get less, or nothing — the Deadlines calculator shows both numbers and warns when the floor cannot fit. `0` divides the budget equally with no floor (the pre-6.10.0 behaviour). |
| `FALLBACK_TOTAL_TIMEOUT` | `600` | Whole-request budget across every attempt, retry and recovery — the backstop for a stream that committed and then stalled. Divided between the models still to try, floored by `FALLBACK_ATTEMPT_SHARE_FLOOR`. `0` disables it. |
| `FALLBACK_STALL_TIMEOUT` | `180` | Seconds a stream that *has* started producing may then go quiet. Measured from the last chunk that moved the answer forward, so keepalives cannot hold a dead stream open and a model producing steadily is never cut. `0` allows an unlimited pause. |
...
| `FALLBACK_REASONING_ANSWER_TIMEOUT` | `450` | Seconds a model may think before the chain moves on. A flat allowance on purpose: the attempt's share of `FALLBACK_TOTAL_TIMEOUT` is sized for a model that has shown *nothing*, and 600s split across an eleven-model chain leaves 54s, far too little to think in. `0` lets a thinking model run to the whole request budget. |
...
**The number in the box used not to be the number you got.** The allowance is `min(first-token deadline, this attempt's share of the total budget)`, and that share is `total ÷ models still to try`. On a ten-model Opus chain with a 600 s budget the field read `120` and each model actually got 60 s — the field was a ceiling, not the allowance. Since 6.10.0 `FALLBACK_ATTEMPT_SHARE_FLOOR` (default `180`, matching the first-token default) stops that share falling below the deadline you set, so the box means what it says; set it to `0` to get the old pure equal-share back.
...
- **`(no key available)`** on an attempt that never reached a credential because every key in the pool was benched. It is not the same as keyless, and the old fabricated `keyless` label is gone. Each attempt names its own key, so a request that rotated mid-route is no longer attributed whole to whichever key happened to finish it.
