...
| **Security** | Token authentication for the local proxy. It is optional only for a loopback bind: with `HOST` accepting connections from other machines and `ANTHROPIC_AUTH_TOKEN` empty, `mcc-server` refuses to start rather than serving an open proxy. |
...
Command Code is the one agent MCC cannot serve without touching a file you own.
It reads exactly one document — `~/.commandcode/providers.json` — and publishes
no flag and no environment variable naming another, so MCC merges a **single
key**, `provider.mcc`, into it. Everything else in that file is read, carried
through and written back byte-for-byte; your document is copied to
`providers.json.mcc-backup` once, before MCC's first edit, and never
overwritten afterwards; and a refresh that resolves the same numbers does not
touch the file at all. Take the key back at any time with:

```bash
mcc-commandcode --disconnect
```

Your proxy token still stays off disk. Command Code refuses a literal key in
`providers.json` and expands a `"$VAR"` reference from the environment instead,
so MCC writes `"$MCC_COMMANDCODE_API_KEY"` and supplies the value to the
launched process only. Inside Command Code, MCC's models are addressed as
`mcc/<provider>/<model>`:

```bash
mcc-commandcode --list-models
mcc-commandcode -p "hello"
mcc-commandcode -m mcc/openrouter/gpt-5 -p "hello"
```
...
Kimi Code is Moonshot's `kimi` CLI — a Python tool on PyPI, installed with `uv
...
not carry over is the rest of your config: because `--config-file` replaces the
document rather than overlaying it, an `mcc-kimi` session takes Kimi's defaults
for `theme`, `hooks` and `loop_control` rather than yours.

This is also the one agent whose generated file holds the proxy token. Kimi's
...
Kimi Code has no model-list subcommand and MCC states no default model, so pick
one on the command line or with `/model` once the session is up:

```bash
mcc-kimi -m mcc/openrouter/gpt-5
mcc-kimi --print -p "hello"
...
>
> **Command Code is the sharpest case, because both halves ship.** The provider `commandcode` on the Providers page is the Command Code *gateway* MCC sends requests to, paid for with a `COMMANDCODE_API_KEY` you bought. `mcc-commandcode` launches the Command Code *CLI*, which sends its requests to MCC. Running `mcc-commandcode` routed to `anthropic` with the `commandcode` provider switched off is a perfectly ordinary setup. The two even have different ids in the registry — the harness is `commandcode_cli` — so no lookup can ever resolve one and answer with the other.
...
| Windows: installing while MCC is running | Supported. The server, the tray, and any open launcher window keep running the version they started with; anything you start afterwards runs the new one. You do not need to close anything. |
| Windows: `These launchers were locked and kept the file they had: ...` | Not an error. Something held those `.exe` files while the installer ran, so they kept their existing launcher — which is a version-agnostic stub that runs the new code anyway. They refresh on the next install. |
| Windows: `Installed, but these commands are missing: ...` | A command the release publishes is genuinely absent. Close the `mcc-claude` window(s) and the tray, then re-run the install command. The installer exits non-zero and never reports "verified" when a command is absent. |
...
### The desktop app, and what happens on first launch (6.44.0)

Since 6.44.0 the first thing `mcc-desktop` tries is **My Claude Code's own desktop app** — a ~1.5 MB window built from `desktop-shell/` in this repository. `uv tool install` cannot deliver a compiled binary, so Python fetches it:

1. It downloads `SHA256SUMS-desktop-shell.txt` from the pinned GitHub release and checks that the digest published there is **the same digest pinned in the wheel** (`config/desktop_shell.py`). If they disagree, the fetch stops — a replaced release asset cannot change what runs on your machine.
2. It downloads the ~1.5 MB archive for your platform, verifies its SHA-256, and extracts exactly one executable from it (no symlinks, no `..` paths).
3. It installs it at `~/.local/bin/MyClaudeCode` (`MyClaudeCode.exe` on Windows) and writes `MyClaudeCode.receipt.json` beside it naming the release tag and digest. **The receipt is why this happens once**: an unchanged pin never downloads again, and a moved pin always does.

If any of that fails — no network, a proxy, an architecture nothing is built for, a read-only home — the launch is **not blocked**. You get a one-line warning naming the reason and the window falls through to Chromium app-mode exactly as it did before.
...
**One tray icon, not two.** On Windows and macOS the Python tray keeps the status-area icon and the app's own tray is switched off; the tray's *Open Admin* raises the existing window rather than opening a second one. On Linux there is no Python tray, so the app's is the only one. The app's tray takes over everywhere in a later release.

### The window chain, and why app-mode is still in it

There is no single "native window" API that works the same way across Windows, macOS, and Linux — WebView2, WKWebView, and WebKitGTK all behave differently. So `mcc-desktop` resolves its window through a **provider chain**. `auto` walks `shell → app-mode → pywebview → browser`.

Chromium **app-mode** — a real browser process launched with no tabs and no URL bar, its own taskbar entry, and a private profile under `~/.mcc/desktop-profile` — is the fallback, and it is a good one. Three things the dashboard depends on **break inside an embedded webview**: `window.open` (both OAuth logins use it), `<a download>` (the analytics export), and `navigator.clipboard` (every copy button). App-mode is a real browser process, so all three work normally.
...
The Models page writes both forms for you, through **one** mechanism: pick rows in the left gutter and press a button in the action bar. That is the write path for three hundred rows and for one — the word beside each checkbox is a readout of the result (`Shown`, `Hidden`, `Hidden by nous_portal/*`), not a second control. Hiding a whole provider writes one glob, `nous_portal/*`, so models it publishes later are hidden too; hiding a selection, a filtered subset, or inverting writes exact refs. Hide all *shadows* the exact patterns underneath rather than deleting them, so the first Show all afterwards lifts only the glob and gives you back exactly the per-model state you had; a second Show all clears those too. **Migrate exact patterns to globs** folds every wholly hidden provider into one `provider/*` — previewed, proved to hide exactly the same models, and undoable. See [Hiding models](docs/USAGE.md#hiding-models) for the three rules.
...
**Both safety nets below run for every provider, not just the OpenAI-compatible ones.** Since 6.33.0 one implementation serves the whole fleet: the Anthropic Messages family (`anthropic`, the Claude subscription, Command Code's Claude models), the ChatGPT OAuth Responses endpoint and every OpenAI-compatible host read the same 400 through the same echo-safe matcher. They were OpenAI-chat-only until then, so the same rejection a custom provider recovered from was a hard failure on a first-party Anthropic key.

**Every OpenAI-compatible host now declares the standard `reasoning_effort` field by default.** That field is defined by the Chat Completions API itself, so a host claiming compatibility either reads it or ignores it — and roughly twenty hosts that previously sent nothing at all now speak it. Which *models* are actually sent it is still the capability gate's decision above, so a mixed roster is protected per model rather than by silencing the whole host. A host that genuinely refuses the field says so with a 400 naming it: MCC strips the field, retries that one request without it, and never asks for it again on that model. A 400 that names a sampling parameter instead is *not* treated as a reasoning rejection — it is raised, because dropping thinking would not have fixed it, and an unrecognised 400 fails visibly rather than being absorbed. The strip is the last retry rung, and it is remembered only on a retry that actually *succeeded*. That memory lives for the life of the process: a config reload rebuilds the provider and forgets it, which is deliberate — a host that was briefly broken heals by itself instead of staying blacklisted. The same is now true of an Anthropic-protocol host that refuses a `thinking` object, and of a Responses host that refuses a `reasoning` block.

The second net is the same idea applied to a number rather than a field. A host that caps output tokens below what was asked for answers with a 400 that **states its own maximum** — `max_completion_tokens must be less than or equal to 40960` on an OpenAI-compatible gateway, `max_tokens: 100000 > 64000, which is the maximum allowed number of output tokens for …` on an Anthropic one. MCC reads the number, clamps that request, retries once, and clamps proactively from then on. It only ever lowers: a cap the deployment stated outranks a catalogue limit, and a catalogue limit already below it is left alone.
